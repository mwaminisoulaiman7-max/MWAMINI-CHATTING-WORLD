...
createGroupBtn.addEventListener('click', async () => {
    ...
    if (error) {
        ...
        // Owner automatically gets added into group_members as approved
...
groupManageBtn.onclick = async () => {
    ...
    pendings.forEach(p => {
        ...
        div.innerHTML = `
            <div>${p.profiles?.full_name || 'Unknown'} <span class="user-item-username">@${p.profiles?.username}</span></div>
            <button class="action-btn" style="max-width:90px; background:var(--accent); color:white; border:none;">Approve</button>
        `;
        div.querySelector('button').onclick = async () => {
            await supabase.from('group_members').update({ status: 'approved' }).eq('group_id', activeGroup.id).eq('user_id', p.user_id);
            alert('User approved!');
            groupManageBtn.click(); // reloads management array
        };
